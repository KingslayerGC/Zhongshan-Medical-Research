...
total_pvalue_df.to_csv(r'C:\Users\Mac\Desktop\过程\项目\total_wilcoxon_pvalue.csv')
total_pvalue_df=pd.read_csv(r'C:\Users\Mac\Desktop\过程\项目\total_wilcoxon_pvalue.csv',header=0,index_col=0,sep=',',low_memory=False)
# %%
import math
loglist=[]
...
for i in range(len(list(total_pvalue_df.columns))):
    ...
    if total_pvalue_df.iloc[0,i] > 0.1:
        break
...
'''
# 以下计算pvalue
total_pvalue_df=pd.DataFrame(columns=[],index=['p_value'])
for col in list(other_variable_df.columns):
    if col=='death':
        continue
    X1=other_variable_df[other_variable_df['death']==0].drop(['death'],axis=1)[col]
    X2=other_variable_df[other_variable_df['death']==1].drop(['death'],axis=1)[col]
    [s,pvalue] = ss.ranksums(X1,X2)
    total_pvalue_df[col]=pvalue
'''
...
#total_df.to_csv(r'C:\Users\Mac\Desktop\过程\项目\total_matrix_wil0.1.csv')
#%%
total_df.to_csv(r'C:\Users\Mac\Desktop\过程\项目\total_matrix_wil0.1.csv')